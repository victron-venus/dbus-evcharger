...
def template_response(
    status="charging", power="3800", current="16", energy="12.5",
    session_time="3600", startstop="1", setcurrent="16",
):
    payload = {
        "status": status,
        "power": power,
...
def test_build_template_falls_back_to_placeholders():
    t = build_template(
        "sensor.evcharger_status",
        "sensor.evcharger_power",
        "sensor.evcharger_current",
    ...
    assert "__no_session_time__" in t
    assert "__no_startstop__" in t
    assert "__no_setcurrent__" in t
...
def test_unconfigured_client_shortcircuits():
    c = HaClient(
        base_url="",
        token="",
        status_entity="sensor.evcharger_status",
